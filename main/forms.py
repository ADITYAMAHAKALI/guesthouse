...
class ReservationForm(forms.Form):
    ...
    error_messages = {
        ...
        'No_of_rooms_invalid':'No of rooms are not equal to Selected Rooms',
        'Less_no_of_rooms':'Selected rooms can not afford all guests ,Please increase no of rooms ',
    }
    first_name = forms.CharField(
        label=_("First Name"),
        max_length=50,
    ...
    widget=forms.TextInput(
         attrs={
                'class': 'form-control',
                'placeholder': _('Enter identity number aadhar/pan/enrollment'),
            }
    ))
    id_proof=forms.ImageField(required=False)
    ...
    no_of_rooms = forms.IntegerField(
        widget=forms.NumberInput(
            attrs={
                'class': 'form-control',
                'placeholder': _('Enter number of rooms'),
            }
        )
    )
    ...
    def clean(self):
        if(self.cleaned_data.get('no_of_rooms')!=self.cleaned_data.get('rooms').count()):
            raise forms.ValidationError(
                self.error_messages['No_of_rooms_invalid'],
                code='No_of_rooms_invalid',
            )

        if(self.cleaned_data.get('no_of_adults')>self.cleaned_data.get('rooms').count()*2):
            raise forms.ValidationError(
                self.error_messages['Less_no_of_rooms'],
                code='Less_no_of_rooms',
            )
