...
from .models import CheckIn, CheckOut ,CancelReservation
...
class CheckInDetailView(PermissionRequiredMixin, generic.DetailView):
    ...
    template_name = 'payments/guest_detail.html'
...
def confirm_check_in(request,pk):
    reservation =Reservation.objects.get(reservation_id=pk)
    checkin =CheckIn.objects.create(reservation=reservation,user=request.user)
    return redirect('check_in-list')


def confirm_check_out(request,pk):
    checkin =CheckIn.objects.get(id=pk)
    checkout =CheckOut.objects.create(check_in=checkin,user=request.user)
    checkout.save()
    return redirect('check_out-list')



def cancel_reservation(request,pk):
    reservation =Reservation.objects.get(reservation_id=pk)
    checkout =CancelReservation.objects.create(reservation=reservation,user=request.user)
    checkout.save()
    return redirect('check_out-list')
